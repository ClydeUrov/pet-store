...
const CardinCart = ({ product, constants, setProducts }) => {
  const [totalCart, setTotalCart] = useState(product.discount ? product.discount : product.price);
  ...
  useEffect(() => {
    setTotalCart(product.discount ? product.discount : product.price * quantity);
  ...
  const handleDelete = (itemId) => {
    ...
    if (indexToRemove !== -1) {
      const updatedCart = [...existingCart.slice(0, indexToRemove), ...existingCart.slice(indexToRemove + 1)];
  ...
  return (
    <div className={css.card}>
      ...
      <div className={css.body}>
        <p>{product.name}</p>
        <p>
          {constants[1].value} {product.price} {product.discount}
        </p>
